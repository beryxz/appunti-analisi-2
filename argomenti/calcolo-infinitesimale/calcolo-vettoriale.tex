...
\subsubsection*{Operazioni in \texorpdfstring{\(\Rn \)}{Rn}}
...
\begin{proof} di \(\circled{1}\)
    ...
    Il nostro \(\Delta \) deve essere negativo, altrimenti, in qualche punto il polinomio dovrebbe essere negativo ma la funzione di partenza era positiva:

    \begin{align*}
        \Delta = 4{\prods{\ux}{\uy}}^{2} - 4\norm\ux^{2}\norm\uy^{2} & \le 0                                       \\
        4{\prods{\ux}{\uy}}^{2}                                      & \le 4\norm\ux^{2}\norm\uy^{2}               \\
        {\prods{\ux}{\uy}}^{2}                                       & \le \norm\ux^{2}\norm\uy^{2}                \\
